        ...
        if results.Contains(Pg) then
            ...
            try
                ...
                if results.Contains(Contracts) then
                    printfn "\n[Stage 5: Emit] Transpiling to Zod & Agent Context"
                    System.IO.Directory.CreateDirectory("output/openmetadata") |> ignore
                    System.IO.Directory.CreateDirectory("client/src") |> ignore
                    
                    let agentContextJson = Canon.Contracts.AgentContextEmitter.emitContext tables
                    System.IO.File.WriteAllText("output/agent-context.json", agentContextJson)
                    
                    let layerMapMd = Canon.Contracts.LayerMapEmitter.generateMap tables
                    System.IO.File.WriteAllText("output/layer-map.md", layerMapMd)
                        ...
                        for c in t.Columns do
                            if not c.CheckConstraints.IsEmpty then
                                let lattice = 
                                    ...
                                    |> List.map Canon.Introspect.SqlParser.parseConstraint 
                                    |> List.reduce (fun a b -> Lattice.And(a, b))
                                let prov = String.Join(" AND ", c.CheckConstraints |> List.map (fun s -> if s.StartsWith("CHECK ") then s.Substring(6) else s))
                                let tsCode, fidelity = Transpiler.emitValidator $"{t.Name}_{c.Name}" lattice c.IsNullable (Some prov)
                        ...
                        for c in t.Columns do
                            if not c.CheckConstraints.IsEmpty then
                                let lattice = 
                                    ...
                                    |> List.reduce (fun a b -> Lattice.And(a, b))
                                let prov = String.Join(" AND ", c.CheckConstraints |> List.map (fun s -> if s.StartsWith("CHECK ") then s.Substring(6) else s))
                                let ktCode, fidelity = KotlinTranspiler.emitValidator $"{t.Name}_{c.Name}" lattice c.IsNullable (Some prov)
                        ...
                        for c in t.Columns do
                            if not c.CheckConstraints.IsEmpty then
                                let lattice = 
                                    ...
                                    |> List.reduce (fun a b -> Lattice.And(a, b))
                                let prov = String.Join(" AND ", c.CheckConstraints |> List.map (fun s -> if s.StartsWith("CHECK ") then s.Substring(6) else s))
                                let swCode, fidelity = SwiftTranspiler.emitValidator $"{t.Name}_{c.Name}" lattice c.IsNullable (Some prov)
                ...
                if results.Contains(Demo) then
                    ...
                    let tsCode, fidelity = Transpiler.emitValidator "price" exampleLattice false None
