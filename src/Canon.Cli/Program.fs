        ...
        if results.Contains(Pg) then
            ...
            try
                ...
                if results.Contains(RestApi) then
                    ...
                    routes.AppendLine("let webApp =") |> ignore
                    routes.AppendLine("    choose [") |> ignore
                    
                    for t in tables do
                        apiSb.AppendLine($"let get{t.Name}Handler : HttpHandler = text \"GET /{t.Name} - Not Implemented\"") |> ignore
                        apiSb.AppendLine($"let post{t.Name}Handler : HttpHandler = text \"POST /{t.Name} - Not Implemented\"") |> ignore
                        
                        routes.AppendLine($"        route \"/api/{t.Name}\" >=> choose [") |> ignore
                        routes.AppendLine($"            GET >=> get{t.Name}Handler") |> ignore
                        routes.AppendLine($"            POST >=> post{t.Name}Handler") |> ignore
